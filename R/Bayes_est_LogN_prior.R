Bayes_est_LogN_prior = function(y, ini.nu = 1 , S = 5000, mu = 0, sigma.sq = 1, exact = TRUE, seed.no = 1){
  # y : Number of samples
  # ini.nu : initial value of MCMC chain
  # S : Number of MCMC sample
  # (mu,sigma.sq) : Hyperparameters of log-normal prior
  # exact = TRUE : Exact MCMC
  # exact = FALSE : Approx. MCMC
  ...
  if (exact == FALSE){
    {
      ...
      for (s in 1:(S-1)){
        ...
        # B. ESS
        {
          ...
          alpha = function(eta.new, eta.old){
            e = exp(1)
            f1 = (1 + 1/e^eta.new)^(e^eta.new)
            f2 = (1 + 1/e^eta.old)^(e^eta.old)
            f3 = ((exp(eta.new)+1)/2)*sum(log(1 + (y^2)/exp(eta.new)))
            f4 = ((exp(eta.old)+1)/2)*sum(log(1 + (y^2)/exp(eta.old)))
            res = min(exp((N/2)*log(f1/f2) -(f3 - f4)),1)
            return(res)
          }
