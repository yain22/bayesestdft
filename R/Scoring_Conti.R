Scoring_Conti <- function (dataset, higher_better, var1, var2){
  ...
  if (higher_better == "Yes"){
    ...
    temp$score = dplyr::if_else(abs(temp[, var1] - temp[, var2]) < 10^(-10), 0, temp$score)
    temp$score = dplyr::if_else(temp[, var2] - temp[, var1] >= 10^(-10), -1, temp$score)
    temp$score = dplyr::if_else(temp[, var1] %in% NA_real_ | temp[, var2] %in% NA_real_, NA_real_, temp$score)
  ...
  temp$WR_cat = dplyr::if_else(temp$WR_cat %in% "" & temp$score %in% 1, paste(gsub('[[:digit:]]+', '', var1), "winner", sep = " "), temp$WR_cat)
  temp$WR_cat = dplyr::if_else(temp$WR_cat %in% "" & temp$score %in% -1, paste(gsub('[[:digit:]]+', '', var1), "loser", sep = " "), temp$WR_cat)
