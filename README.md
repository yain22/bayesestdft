...
### The user needs to install several R libraries in order to set up winratiosim. Ensure that the following libraries are installed.
```{r}
library(plyr)
library(dplyr)
library(parallel)
...
```
# References
1. Lee, S.Y. A note on the sample size formula for a win ratio endpoint. Statistics in Medicine (provisionally accepted) 2025:00(00):0-00
